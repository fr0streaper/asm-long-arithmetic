...
mul_long_long:
                ...
                push            rdx
                push            rbp ; stores initial position of rsp
                push            rbx ; carry
                push            r8  ; first cycle counter
                push            r9  ; second cycle counter
                push            r10 ; copies of rdi and rsi to write changes
                push            r11

                ; clear CF
                clc
                ; set outer cycle counter to number length
                mov             r8, rcx
                ; make space for the result
                shl             rcx, 4
                sub             rsp, rcx
                shr             rcx, 4        
                ; store current position of rsp
                mov             rbp, rsp
                ; store current values of rdi & rsi
                mov             r10, rdi
                mov             r11, rsi

.first_num_loop:
                ; set inner cycle counter to number length
                mov             r9, rcx
                ; restore rsi original position
                mov             rsi, r11
                ; set carry to zero
                xor             rbx, rbx

                .second_num_loop:
                                ; move current qword at rdi to rax
                                mov             rax, [rsi]
                                ...
                                ; add carry to high part
                                adc             rdx, 0
                                ; move high part with carry to rbx
                                mov             rbx, rdx
                                ; move rsp to next qword in memory
                                add             rsp, 8
                ...
                ; add the carry to memory at rsp
                add             [rsp], rbx
                ; return rsp to initial position
                shl             rcx, 3
                sub             rsp, rcx
                shr             rcx, 3
                ; shift rsp by qword with the change of the qword in number #1
                add             rsp, 8
                ...
                ; move rsp back to initial position (stored in rbp)
                mov             rsp, rbp
                ; restore initial values of rdi & rsi
                mov             rdi, r10
                mov             rsi, r11
                ; set loop counter to product low part size
                mov             r8, rcx
                ...
                ; set loop counter to product high part size
                mov             r8, rcx
